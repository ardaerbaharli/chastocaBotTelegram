            ...
            if (message != null)
            {
                ...
                else if (message.StartsWith("!addlocation") && username.Equals("ardaerbaharli"))  //!addlocation arda lati longi
                {
                    if (fragmentedMessage.Length == 4)
                    {
                        ...
                        bool isSuccessfull = DatabaseHandler.AddLocation(name, latitude, longitude);
                        await botClient.SendTextMessageAsync(e.Message.Chat, "Location added: " + isSuccessfull, ParseMode.Markdown);
                    }
                    else
                    {
                ...
                else if (message.StartsWith("!deletelocation") && username.Equals("ardaerbaharli"))
                {
                    ...
                    {
                        string name = fragmentedMessage[1];
                        bool isSuccessfull = DatabaseHandler.DeleteLocation(name);
                        await botClient.SendTextMessageAsync(e.Message.Chat, "Location deleted: " + isSuccessfull, ParseMode.Markdown);
                    }
                    else
                    {
                ...
                else if (message.StartsWith("!addcommand") && username.Equals("ardaerbaharli"))
                {
                    if (fragmentedMessage.Length > 2)
                    {
                        ...
                        bool isSuccessfull = DatabaseHandler.AddCommand(command, reply);
                        await botClient.SendTextMessageAsync(e.Message.Chat, "Command added: " + isSuccessfull, ParseMode.Markdown);
                    }
                    else
                    {
                ...
                else if (message.StartsWith("!deletecommand") && username.Equals("ardaerbaharli"))
                {
                    ...
                    {
                        string command = fragmentedMessage[1];
                        bool isSuccessfull = DatabaseHandler.DeleteCommand(command);
                        await botClient.SendTextMessageAsync(e.Message.Chat, "Command deleted: " + isSuccessfull, ParseMode.Markdown);
                    }
                    else
                    {
                ...
                else if (message.StartsWith("!addcocktail") && username.Equals("ardaerbaharli")) //!addcocktail name type recipe
                {
                    if (fragmentedMessage.Length > 3)
                    {
                        ...
                        bool isSuccessfull = DatabaseHandler.AddCocktail(cocktailName, recipe, cocktailType);
                        await botClient.SendTextMessageAsync(e.Message.Chat, "Cocktail added: " + isSuccessfull, ParseMode.Markdown);
                    }
                    else
                    {
                ...
                else if (message.StartsWith("!deletecocktail") && username.Equals("ardaerbaharli"))
                {
                    ...
                    {
                        string cocktailName = fragmentedMessage[1];
                        bool isSuccessfull = DatabaseHandler.DeleteCocktail(cocktailName);
                        await botClient.SendTextMessageAsync(e.Message.Chat, "Cocktail deleted: " + isSuccessfull, ParseMode.Markdown);
                    }
                    else
                    {
